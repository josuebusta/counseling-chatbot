...
class ConversableAgent(LLMAgent):
    ...
    # CUSTOM CHECK TERMINATION AND HUMAN REPLY -> takes in user input from the UI
    ...
    async def a_check_termination_and_human_reply(
        ...
        sender_name = "the sender" if sender is None else sender.name
        if self.human_input_mode == "ALWAYS":
            reply = await self.await_human_input_via_websocket(self.websocket)
            
            # await self.a_get_human_input(
            #     f"Replying as {self.name}. Provide feedback to {sender_name}. Press enter to skip and use auto-reply, or type 'exit' to end the conversation: "
            # )
            # no_human_input_msg = "NO HUMAN INPUT RECEIVED." if not reply else ""
            # if the human input is empty, and the message is a termination message, then we will terminate the conversation
