...
# from .accuracy_evaluation import evaluate_counseling_response  # Module not found
...
def classify_response(response, language):
    ...
    prompt = (f"In {language}, classify this response as 'affirmative', 'negative', "
              f"'stop' (if the user wants to stop or exit out of the assessment), "
              f"'clarification', or 'unsure': '{response}'. "
        ...
        for question in QUESTIONS:
            ...
            if classification == "negative":
                ...
                classification, user_response = await handle_clarification(
                    websocket, question, user_response, language)
    ...
    try:
        ...
        try:
            ...
            for result in results:
                ...
                distance = (distance_with_label.replace('Distance from your location:', '')
                            .strip() if distance_with_label != 'N/A' else 'N/A')
        ...
        return translate_question(f"I'm sorry, I couldn't find any providers near you. "
...
async def assess_ttm_stage_single_question(websocket: WebSocket,
                                           language: str) -> str:
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                await websocket.send_text(translate_question(
                    "I didn't understand that. Please respond with 0, 1, or 2.", 
                    language))
        ...
        if contact_preference == "1":
            ...
            contact_info_prompt = ("Please provide your email address so that a "
                                  "research assistant can reach out to you.")
        await websocket.send_text(translate_question(contact_info_prompt,
                                                     language))
